 ...
 *
 * dcyfr-bot's instance of this defect was already repaired by hand (see the
 * comment above the Send button in `components/ChatInterface.tsx`). The gate
 * is ported so the next one is caught by CI rather than by a sweep.
 *
