...
export default async function AgentDetailPage({ params }: PageProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <Link href="/agents" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-muted-foreground mb-8 transition-colors">
        ← Back to Agents
      </Link>
      ...
      <div className="bg-card/30 border border-border/40 rounded-xl p-6 sm:p-8 mb-8">
        ...
        <p className="text-muted-foreground mb-6">{agent.longDescription ?? agent.description}</p>
      ...
      {rep && (
        <div className="bg-card/20 border border-border/30 rounded-xl p-6 mb-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-center">
            ...
            <div><p className="text-2xl font-bold text-muted-foreground">{rep.totalRatings}</p><p className="text-xs text-muted-foreground mt-0.5">Ratings</p></div>
            <div><p className="text-2xl font-bold text-muted-foreground">{(rep.successRate * 100).toFixed(0)}%</p><p className="text-xs text-muted-foreground mt-0.5">Success Rate</p></div>
          </div>
        </div>
      )}
